        ...
        >
            <span className="suite-card-head">
                ...
                <span className={'suite-status status-' + suite.data_status}>
                    {suite.collection_enabled ? t.status[suite.data_status] ?? suite.data_status : t.collectionOff}
                </span>
            </span>
            ...
            <span className="suite-unit">{t.units[suite.unit] ?? suite.unit}</span>
            ...
            {hasContract ? (
                <span className="suite-contract">
                    ...
                    <span className="suite-contract-text">
                        {formatNumber(suite.contract_quantity)} {t.entitled}
                        {overCommitted ? ' · ' + formatNumber(suite.overage) + ' ' + t.over : ''}
                    </span>
                </span>
            ) : (
                <span className="suite-contract-text is-muted">{t.noEntitlement}</span>
            )}

            {suite.reclaimable > 0 ? (
                <span className="suite-reclaimable">{t.reclaimableNote(formatNumber(suite.reclaimable))}</span>
            ) : null}
        </button>
    )
}

export default function SuiteGrid({ suites, selected, onSelect, t }: Props) {
    return (
